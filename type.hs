...
lookUp :: Eq a => a -> [(a, b)] -> b
lookUp x y = tryToLookUp x undefined y

tryToLookUp :: Eq a => a -> b -> [(a, b)] -> b
tryToLookUp x d [] = d 
tryToLookUp x d (y:ys)
  | x == (fst y) = snd y
  | otherwise    = tryToLookUp x d ys 

-- Pre: The given value is in the table
reverseLookUp :: Eq b => b -> [(a, b)] -> [a]
reverseLookUp x y = [i | (i, j) <- y, j == x] 

occurs :: String -> Type -> Bool
occurs x (TVar y) = x == y
occurs x (TFun o1 o2) = (occurs x o1) || (occurs x o2)
occurs x y = False 
...
inferType (Id      x)  e = lookUp x e
inferType (Prim    x)  _ = lookUp x primTypes
inferType (Cond (Boolean _) x y) e
  | inferType x e == inferType y e = inferType x e
  | otherwise = TErr
inferType (Cond _ _ _) _ = TErr
inferType (App f a) e
  | inferType f e == TErr = TErr
  | x == inferType a e = y
  | otherwise = TErr
  where (TFun x y) = inferType f e 
...
applySub s (TFun a b) = TFun (applySub s a) (applySub s b)
applySub _ t = t
...
unifyPairs ((TVar v, TVar v'):xs) s
  | v == v' = unifyPairs xs s
  | otherwise = unifyPairs ys ((v, TVar v'):s)
  where ys = map (\(t1, t2) -> (applySub [(v, TVar v')] t1, applySub [(v, TVar v')] t2)) xs
unifyPairs ((TVar v, t):xs) s
  | occurs v t = Nothing
  | otherwise = unifyPairs ys ((v, t):s)
  where ys = map (\(t1, t2) -> (applySub [(v, t)] t1, applySub [(v, t)] t2)) xs
